...
typedef struct	
{
	...
	int (*ShowQrCode)(RECTL*,const char*,A_RGB);	//(RECTL *pRect,const char* pInfo,A_RGB Color)
	...
	int (*FixedMenu)(char*,const FixedMenuUITable *,void*);		//(char* pTitle,const FixedMenuUITable *pFixedMenu,void* pTagFunc)
		
	int (*CreateMenuByStr)(char *,int,char**,const APP_IndexH,int);	//(char *pTitle,int tNum,char* *pMenuText,const APP_IndexH pUnifiedFunc,int TimeOutMs)
	int (*CreateMenuByStruct)(char *,int,CMenuItemStru *,int);	//(char *pTitle,int tNum,CMenuItemStru *pMenuAll,int TimeOutMs)
	int (*CreateNewMenuByList)(char* ,CMenuListPar*,int);	//(char* pTitle,CMenuListPar* pMenulist,int TimeOutMs)
	void (*AddCurrentMenuOtherFun)(UI_MENU_ADD_TYPE,void*,const char *);	//(UI_MENU_ADD_TYPE type,void* pFunc,const char *pData)
